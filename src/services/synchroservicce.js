...
export default {
    async checkAndSyncData() {
        ...
        if (clientsToSync.length > 0) {
            await this.syncData('https://back3a.fl4ir.com/api/auth/sync-clients', clientsToSync, 'clients');
        }

        if (prospectsToSync.length > 0) {
            await this.syncData('https://back3a.fl4ir.com/api/auth/sync-prospects', prospectsToSync, 'prospects');
        }

        if (branchesToSync.length > 0) {
            await this.syncData('https://back3a.fl4ir.com/api/auth/sync-branches', branchesToSync, 'branches');
        }

        if (compagniesToSync.length > 0) {
            await this.syncData('https://back3a.fl4ir.com/api/auth/sync-compagnies', compagniesToSync, 'compagnies');
        }

        if (apporteursToSync.length > 0) {
            await this.syncData('https://back3a.fl4ir.com/api/auth/sync-apporteurs', apporteursToSync, 'apporteurs');
        }

        if (contratsToSync.length > 0) {
            await this.syncData('https://back3a.fl4ir.com/api/auth/sync-contrats', contratsToSync, 'contrats');
        }
    },
